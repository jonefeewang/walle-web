    ...
    <?php foreach ($group as $relation) { ?>
        <div class="itemdiv memberdiv">
            <div class="inline position-relative">
                ...
                <div class="body">
                    <div class="name">
                        <?php if ($relation['type'] == Group::TYPE_REVIEWER) { ?>
                    ...
                    <div class="popover-content">
                        ...
                        <div class="tools action-buttons">
                            <a href="javascript:;" class="bind-admin" data-id="<?= $relation['id'] ?>"
                               data-type="2"
                               data-action="<?= $relation['type'] == Group::TYPE_REVIEWER ? 'remove' : 'add' ?>"
                               title="<?= yii::t('conf', 'audit manager tip') ?>">
                                <i class="icon-user-md light-orange bigger-110"></i>
                                <?= $relation['type'] == Group::TYPE_REVIEWER ? yii::t('conf', 'cancel audit manager') : yii::t('conf', 'add audit manager') ?>
                            </a>
                        </div>
                        <div class="tools action-buttons">
                            <a href="javascript:;" class="bind-admin" data-id="<?= $relation['id'] ?>"
                               data-type="1"
                               data-action="<?= $relation['type'] == Group::TYPE_DEV ? 'remove' : 'add' ?>"
                               title="<?= yii::t('conf', 'audit manager tip') ?>">
                                <i class="icon-user-md light-orange bigger-110"></i>
                                <?= $relation['type'] == Group::TYPE_DEV ? yii::t('conf', 'cancel dev') : yii::t('conf', 'add dev') ?>
                            </a>
                        </div>
                    </div>
                </div>
